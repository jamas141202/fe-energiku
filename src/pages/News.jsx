  ...
  return (
    <div className="rounded-lg mx-4 sm:mx-[50px] md:mx-[150px] mt-[200px]">
      ...
      >
        <div className="flex flex-col items-center w-full px-2">
          {newsCategories.slice(0, 2).map((category, index) => (
            <motion.div
              key={category.id}
              className="hover:opacity-80 rounded-lg p-2 cursor-pointer mb-4 w-full"
              style={{ height: 'auto' }}
      ...
      <div className="p-2 sm:p-6 max-w-7xl mx-auto">
        <motion.h2
          className="text-2xl sm:text-3xl font-bold mb-2 text-yellow-500"
          initial="hidden"
          animate="visible"
          variants={fadeIn}
          transition={{ duration: 2 }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {videoSections.flatMap((section) =>
            section.videos.map((video, index) => (
              <motion.div
                key={index}
                className="bg-gray-100 p-3 sm:p-4 rounded-lg shadow-md"
                variants={slideInFromBottom}
                transition={{ duration: 2, delay: index * 0.3 }}
              >
                <iframe
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  <img src={Image5} alt="YouTube Logo" className="w-8 sm:w-12 h-8 sm:h-12" />
                  <p className="text-sm sm:text-base text-gray-800 font-semibold">{video.title}</p>
                </div>
              </motion.div>
            ))
          )}
